...
# ---------------------------------------------------------------------------
# THEME / STYLING — professional light theme, custom font, polished cards
# ---------------------------------------------------------------------------
...
/* Expanders (invoice cards in Live Feed) */
...
# Consistent Plotly styling applied to every chart before rendering
def style_chart(fig):
    fig.update_layout(
...
<div style="display:flex; align-items:center; gap:14px; margin-bottom:2px;">
    <div style="background:#0F172A; width:52px; height:52px; border-radius:14px;
                display:flex; align-items:center; justify-content:center; font-size:26px;">🛡️</div>
    <div>
        <div style="font-size:1.8rem; font-weight:800; color:#0F172A; letter-spacing:-0.02em;">PayHoplon</div>
        <div style="color:#64748B; font-size:0.95rem;">Autonomous invoice reconciliation &amp; UPI payment release, inside guardrails you control.</div>
    </div>
</div>
<div style="height:1px; background:#E2E8F0; margin:18px 0 22px 0;"></div>
""", unsafe_allow_html=True)
...
with tab4:
    if total == 0:
        ...
        with row1c1:
            ...
            color_map = {
                "Auto-Approved & Released": "#16A34A",
                "Cleared — Awaiting Sign-off": "#D97706",
                "Flagged for Review": "#DC2626",
                "Manually Approved": "#2563EB",
                "Rejected": "#94A3B8",
            }
            fig_pie = px.pie(
                decision_counts, names="Decision", values="Count",
                title="Decision breakdown", hole=0.55,
        ...
        with row1c2:
            ...
            fig_cum.update_traces(line_color="#16A34A", line_width=3, marker=dict(size=6, color="#16A34A"))
        ...
        with row2c1:
            ...
            fig_hist.add_vline(
                ...
                line_color="#DC2626", annotation_text="Budget cap",
            )
            fig_hist.add_vline(
                x=st.session_state.guardrails["auto_approve_ceiling"], line_dash="dash",
                line_color="#16A34A", annotation_text="Auto ceiling",
            )
        ...
        with row2c2:
            ...
            st.plotly_chart(style_chart(fig_vendor), use_container_width=True)
        ...
        if all_reasons:
            ...
            fig_reasons = px.bar(
                ...
                color="Reason",
                color_discrete_sequence=["#DC2626", "#D97706", "#94A3B8", "#2563EB", "#0F172A", "#16A34A"],
            )
